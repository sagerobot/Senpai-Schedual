 ...
 *
 * `presetSummary` short-circuits the AI request entirely. The season bundle
 * carries summaries that were generated by the same prompt against the same
 * synopsis, so a bundled show needs no round-trip and cannot come back
 * degraded — which is also why the bundle, not the server's in-memory LRU, is
 * what makes summaries survive a restart.
 */
export async function fetchShowDetails(
  ...
  if (presetSummary !== undefined) {
    details.aiSummary = presetSummary;
    details.aiStatus = 'ok';
  } else {
