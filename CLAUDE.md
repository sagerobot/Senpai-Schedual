...
**The wake strip** (`src/components/WakeStrip.tsx`, driver `src/queries/serverWake.ts`, pure machine `src/lib/wakeStrip.ts`) is the in-page answer to "is this the current version?". The service worker paints the shell instantly from the device's copy while the free-tier host is asleep, and the worker's own update check runs once at page load — exactly when the server can't answer — so a deploy was invisible until the next visit. The driver registers the worker itself (`injectRegister: false` in `vite.config.ts`), probes `/api/health` until the host answers, then compares **build stamps**: `server/buildStamp.ts` resolves `RENDER_GIT_COMMIT` → `git rev-parse` → `"dev"` on both sides (baked into the client as `__APP_BUILD__` via `define`, reported by the server in `/api/health.build`). Same stamp → a 3s green confirmation, then hidden. Newer → `registration.update()` and one reload under `src/lib/reloadGuard.ts` (one per minute per reason, sessionStorage, shared with RouteError's stale-chunk heal), so it can never loop; a refused reload becomes a button. A `dev` stamp on either side never counts as an update. Settings' "About this build" section shows both stamps and the manual check. Phases and copy live in the pure machine and are tested there.

- **Today's Drops audience** is one rule, `src/lib/dropAudience.ts`:
  - watching shows get every episode
  - stacking shows only their finale
  - Plan to Watch shows, and **guest** season premieres (`hooks/useGuestPremieres.ts`: a franchise you follow whose new entry isn't in the library), only episode 1
...
- **Hero card headers share three primitives** (`src/components/`): `FitTitle` (a fixed two-line slot; the font shrinks to fit the whole name, never an ellipsis — the slot grows only if the floor size still overflows), `CardMetaPills` (progress + Your Avg, right-aligned on their own row), `RatingBlock` (header left, 6-column score grid, Watched only / 0-4 beneath). Drops and deck cards draw them at the same y so a card row lines up; what varies between cards sits between the pills and the CTA. Don't reintroduce `line-clamp` on a title — Tailwind's `block` overrides its display and the clamp silently dies.
...
- **`cn()` (`src/lib/utils.ts`) is tailwind-merge extended with the custom text sizes** (`text-micro` / `text-caption` / `text-label`). Without that extension tailwind-merge files them under text *color* and a later `text-fg-muted` silently deletes the size — add any new `@theme` font size there too.
...
- **Every AniList request must go through `src/api/anilist/client.ts`.** It owns the module-level rate limiter (concurrency 1, 650ms min gap, paced by AniList's CORS-exposed `X-RateLimit-Remaining` against its 30/min window, global pause on a 429 *or* on a rejected fetch) shared by every caller, checks both `response.ok` *and* GraphQL `errors`, and caps retries at 4. Bypassing it reintroduces the request storms this design makes structurally impossible. **AniList's 429 responses carry no CORS headers**, so in a browser a rate limit surfaces as a `TypeError` from `fetch`, never as `status === 429` — the client treats that rejection as a rate limit and backs off; a version that didn't fired 79 requests into one closed window.
- **Paginated walks stop on a short page, never on `hasNextPage` alone** (`src/api/anilist/pagination.ts`, used by the client and the bundle build). AniList's flag undercounts; trusting it shipped three weeks of half-size season bundles in Aug 2026. The show-count gate in `scripts/seasonGates.ts` is asymmetric (−30% blocks, growth passes up to 3×) for the same incident.
